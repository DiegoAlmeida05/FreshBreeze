...
const BUCKET_NAME = 'property-key-photos'
...
const keyPhotoHandler = defineEventHandler(async (event) => {
  ...
  const keyPhotoUrl = publicUrl
    ? `${publicUrl}${publicUrl.includes('?') ? '&' : '?'}v=${Date.now()}`
    : null

  if (!keyPhotoUrl) {
    throw createError({ statusCode: 500, statusMessage: 'Failed to generate key photo URL.' })
  }

  return {
    success: true,
    keyPhotoUrl,
    message: 'Key photo uploaded successfully.',
  }
})

export default keyPhotoHandler
